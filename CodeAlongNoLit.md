...
1.  This step we populate the repositories page with real data.
    ...
    6. Then move the existing table into a `<template>` with the id `template--table`
    ...
    7. In `main.js` we next add this replaceSkeleton function
    ...
    8. Still in `main.js` call the Octokit API and replace our data array.
...
2.  Fixing the links column.

    1. Import the `cds-link` component in `main.js`.

       ```js
       import '@carbon/web-components/es/components/link/index';
       ```

    2. In the `fetchData` function replace 'links' with

       ```js
       links: { url: row.html_url, homepage: row.homepage },
       ```

    3. In our update table function we need to do something different for the links key. Replace
...
3.  Adding pagination
    ...
    1.  In `main.js` as the pagination component

        ```js
        import '@carbon/web-components/es/components/pagination/index';
        ```

    2.  Where we declare the data variable, add variables for page size and row index.
